...
if(preg_match('~\.(jpg|jpeg|gif|png|bmp|js|css|txt|jar|tgz|rar|zip|gz)$~i', $_SERVER['REQUEST_URI']))// If you whant to hide orign host, better delete next code
{
  header("Location: ".$url);
  exit;
}
...
foreach($header as $key=>$value){
	if(!preg_match("~^transfer-encoding~i",$value)) {
		$value = str_replace(array($base, $base_http_host), array($mydomain,$_SERVER['HTTP_HOST']), $value);
		header(trim($value));
	}
}
